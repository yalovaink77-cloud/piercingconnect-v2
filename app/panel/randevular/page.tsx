        ...
        {randevular.map(r => (
          <div key={r.id} className="bg-gray-800 rounded-2xl p-4 mb-3">
            <div className="flex justify-between items-start mb-1">
              <p className="text-white text-lg font-semibold">{r.name || 'Isimsiz'}</p>
              <span className={r.status === 'onaylandi' ? 'text-green-400 text-sm' : r.status === 'iptal' ? 'text-red-400 text-sm' : 'text-yellow-400 text-sm'}>
                {r.status}
              </span>
            </div>
            {r.phone && <p className="text-orange-400 text-sm mb-1">{r.phone}</p>}
            <p className="text-gray-500 text-sm mb-1">{r.service_type}</p>
            {r.notes && <p className="text-gray-400 text-sm mb-2">{r.notes}</p>}
